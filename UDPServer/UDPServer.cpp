...
DWORD WINAPI threadFun(LPVOID param)
{
    ...
    while (serve)
    {
        ...
        {
            case RETURN_ROOMS:
                response = new DataPacket(clientPacket.client_id, clientPacket.sequence, clientPacket.function, videoGame.currentRoom, videoGame.maxRooms, videoGame.playerMoney, videoGame.playerKeys, videoGame.character->health, videoGame.character->maxHealth, videoGame.enemy->name, videoGame.enemy->health, videoGame.enemy->maxHealth, videoGame.roomGenerated);
